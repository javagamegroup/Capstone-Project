    ...
    public void collision(){
        ...
        for (int i = 0; i < World.getAreas().size(); i++) {
            ...
            if (this.playerRect.intersects(area.areaRect)) 
            {
            	...
            	{
            		try {GamePanel.level.setPlayerEntrance('e',GamePanel.level);} catch (InvalidCharacterException e) {e.printStackTrace();}
            		this.setLevel(true);
            	}
            	else if(playerRect.x< 400 && xDirection ==-1)
            	{
            		try {GamePanel.level.setPlayerEntrance('w',GamePanel.level);} catch (InvalidCharacterException e) {e.printStackTrace();}
            		this.setLevel(true);
            	}
            	else if(playerRect.y> 200 && yDirection ==1)
            	{
            		try {GamePanel.level.setPlayerEntrance('s',GamePanel.level);} catch (InvalidCharacterException e) {e.printStackTrace();}
            		this.setLevel(true);
            	}
            	else if(playerRect.y< 400 && yDirection ==-1)
            	{
            		try { GamePanel.level.setPlayerEntrance('n',GamePanel.level);} catch (InvalidCharacterException e) {e.printStackTrace();}
            		this.setLevel(true);
            	}
            }
        }
